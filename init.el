...
;;            c-c++-enable-clang-format-on-save t) TODO: 这里会让php 保存的时候也会clang-format 解决下
   ...
   dotspacemacs-excluded-packages '(wolfram-mode
                                    pangu-spacing ;;org中英文之间有空格导致deft标题显示不齐
                                    ;; phpcbf
                                    )
...
If you are unsure, try setting them in `dotspacemacs/user-config' first."
  ...
  (setq exec-path-from-shell-check-startup-files nil) ;;去掉path 警告

  ;;编译package的警告不显示
  (setq byte-compile-warnings '(not obsolete))

  ;;mac的/bin/sh 并不是/bin/bash的symlink 而是兼容posix的版本 所以没法读取.bashrc(nologin) 和.bash_profile(login)文件
  ;; (setenv "ENV" "$HOME/.bashrc")

  ;;必须写在user-init里 因为spacemacslayer里有检查是在初始化的时候
  ;; (setq ycmd-server-command (list "python" (file-truename "~/Documents/code/thirdparty/ycmd/ycmd")))

  ;;设置https错误验证域名 可能还有问题再看
  ;; (setq gnutls-verify-error '(("sandbox\\.google\\.cn" nil)))
  ...
  ;;   (let ((fcitx-path "~/Dropbox/Apps/emacs/win-tools"))
  ...
  ;;加载敏感数据
  (setq secrets-file (expand-file-name ".emacs.secrets.el" dotspacemacs-directory))
  (load secrets-file t)
  )
...
before packages are loaded."
  ...
  ;;       explicit-cmd-args '("/q"))
  ;;ycmd
  ;; (setq ycmd-extra-conf-whitelist '("~/Documents/code/*"))
  ...
  ;; 设置路径 防止gtd 报错
  ;;https://github.com/syl20bnr/spacemacs/issues/9941#issuecomment-348604479
  ...
  ;;mssql
  ;;https://github.com/sebasmonia/sqlcmdline 注意diver 的名字要改下再编译成二进制不然会报错
  ;;https://github.com/sebasmonia/emacs_cfg/blob/master/init.el  
  ;;https://www.emacswiki.org/emacs/SqlMode
  ;;https://truongtx.me/2014/08/23/setup-emacs-as-an-sql-database-client
  (setq sql-ms-program "sqlcmdline")
