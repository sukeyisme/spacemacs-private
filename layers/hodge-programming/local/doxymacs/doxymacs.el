;;; -*-emacs-lisp-*-
;;; doxymacs.el.in --- ELisp package for making doxygen related stuff easier.
;;
;;
...
(defface doxymacs-keyword-face
  '((t (:italic t :foreground "#6a5acd")))
  "Face to highlight doxygen keywords"
  :group 'font-lock-highlighting-faces)
(defvar doxy-keyword-face 'doxymacs-keyword-face)
;;(setq doxy-keyword-face 'doxymacs-keyword-face)
  ...
  (list
   (list
    ...
    (concat "\\([@\\\\]\\(brief\\|li\\|\\(end\\)?code\\|sa"
	    ...
	    ;;"\\|\\\\&\\|\\$\\|\\#\\|<\\|>\\|\\%"
	    "\\|\\$"
	    "\\|internal\\|nosubgrouping\\|author\\|details\\|date\\|endif"
...
(defconst doxymacs-C++-file-comment-template
 '("/**" > n
   ...
   (doxymacs-user-mail-address)
   > n
   "* " (doxymacs-doxygen-command-char) "date   " (format-time-string "%Y-%m-%d %H:%M") > n
   "* " (doxymacs-doxygen-command-char) "version   " "0.0.1"> n
   "* " (doxymacs-doxygen-command-char) "brief  " (p "Brief description of this file: ") > n
   "* " > n
   "* " (doxymacs-doxygen-command-char) "details  " (p "Detail description of this file: ") > n
   "*/" > n)
